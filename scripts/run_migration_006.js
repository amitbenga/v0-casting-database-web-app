...
SELECT
  ...
  -- Direct subqueries avoid the project_scripts dependency bug
  COALESCE((SELECT COUNT(*) FROM script_lines sl2 WHERE sl2.project_id = p.id), 0)::int AS total_lines,
  COALESCE((SELECT COUNT(*) FROM script_lines sl3 WHERE sl3.project_id = p.id AND sl3.rec_status = 'הוקלט'), 0)::int AS recorded_lines,
